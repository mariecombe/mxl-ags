...
class model:
  ...
  def initmodel(self):
    ...
    self.T0         =  273.15                # temperture reference [K]
    ...
    self.RHh        =  -1.                   # relative humidity top of the CBL
  ...
  def runmlmodel(self):
    if(not self.sw_sl):
      ...
      u2lim = max(self.u**2., 1.e-4)
      v2lim = max(self.v**2., 1.e-4)
      usign = numpy.math.copysign(1.0, self.u)
      vsign = numpy.math.copysign(1.0, self.v)
      self.uw       = - usign * (self.ustar ** 4. / (v2lim / u2lim + 1.)) ** (0.5)
      self.vw       = - vsign * (self.ustar ** 4. / (u2lim / v2lim + 1.)) ** (0.5)
...
#    print(self.t,tutc,self.advtheta)
    ...
    Ptop     = self.P0 / numpy.exp((self.g * self.h)/(self.Rd * self.theta))
    Ttop     = self.theta / ((self.P0/ Ptop) ** (self.Rd / self.cp))
    estop    = 611. * numpy.exp((self.Lv / self.Rv)*((1. / self.T0)-(1. / Ttop)))

    qstop    = (0.622 * estop) / Ptop
    self.RHh = (self.q / qstop)
...
#    self.lcl    = 125*(self.theta-Tdew) 

    # NEW Iterative find LCL
    lcl      = self.h        # first guess...
    RHlcl    = 0.95          # another guess.....

    while (RHlcl <= 0.999) | (RHlcl >= 1.001):
      if RHlcl < 0.999:
        if RHlcl < 0.85:
          lcl = lcl + 50.
        else:
...
#      Ptop         = self.Ps / numpy.exp((self.g * lcl)/(self.Rd * self.theta))
#      Ttop         = self.theta / (self.PS / Ptop)**(self.Rd / self.cp)
#      esattop      = 0.611e3 * numpy.exp((self.Lv / self.Rv) * ((1. / 273.15)-(1. / Ttop)));
#      etop         = self.q * Ptop / 0.622;
      Ptop        = self.Ps / numpy.exp((self.g* lcl) / (self.Rd * self.theta))
      Ttop        = self.theta * (Ptop / self.Ps ) ** (self.Rd / self.cp)
      esattop     = 0.611e3 * numpy.exp(17.2694 * (Ttop - 273.16) / (Ttop - 35.86)) 
  ...
  def runlsmodel(self):
    ...
    if(sw_rsAgs): 
      ...
      self.Resp   = self.R10 * (1 + self.ustar)* (1. - self.fw) * numpy.exp( self.E0 / (283.15 * 8.314) * (1. - 283.15 / (self.thetasurf)))  
...
#      print(self.An,self.Resp)
  ...
  def store(self):
    ...
    self.out.co2tend[t]    = self.co2tend  
...
class modeloutput:
  def __init__(self, tsteps):
    ...
    self.we         = numpy.zeros(tsteps)    # entrainment velocity [m s-1]
    ...
    self.co2tend    = numpy.zeros(tsteps)    # tendency carbon dioxide [ppm s-1] 
    ...
    self.RHh        = numpy.zeros(tsteps)    # Relative humidty at h top
